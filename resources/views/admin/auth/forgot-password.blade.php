...
<head>
    ...
    <title>{{ __('admin.forgot_pw_heading') }} — BARQAWI Admin</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="min-h-screen bg-slate-100 flex antialiased font-sans">

{{-- Panneau décoratif gauche (identique à la page de connexion) --}}
<div class="hidden lg:flex lg:w-96 xl:w-[440px] bg-slate-900 flex-col justify-between p-10 shrink-0 relative overflow-hidden">
    ...
    <div class="relative">
        <h1 class="text-3xl font-extrabold text-white leading-tight mb-4">
            <span class="text-blue-400">{{ __('admin.forgot_pw_tagline') }}</span>
        </h1>
        <p class="text-white/50 text-sm leading-relaxed">{{ __('admin.forgot_pw_desc') }}</p>
    </div>
...
<div class="flex-1 flex items-center justify-center p-6 min-h-screen">
    <div class="w-full max-w-sm">
        ...
        <div class="mb-8">
            <h2 class="text-2xl font-extrabold text-slate-900 mb-1">{{ __('admin.forgot_pw_heading') }}</h2>
            <p class="text-sm text-gray-500">{{ __('admin.forgot_pw_subtitle') }}</p>
        </div>
